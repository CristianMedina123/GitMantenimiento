...
$querySelectEstadoEquipo = mysqli_query($conn, "SELECT idtipoestado, tipoestado FROM tipoestado");
			...
			<div class="row">
				<div class="col-xs-12">
					<ul class="nav nav-tabs" style="margin-bottom: 15px;">
					  	...
					  	<li><a href="#list" data-toggle="tab">Lista de los Estados</a></li>
					</ul>
						...
						<div class="tab-pane fade active in" id="new">
							<div class="container-fluid">
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
									    <form id="frmTipoEstado" method="post">
									    	<div class="form-group label-floating">
											  <label class="control-label">Tipo de Estado</label>
											  <input class="form-control" type="text" id="txtTipoEstado" autocomplete="off">
											</div>
										    <p class="text-center">
										    	<button type="button" onclick="InsertarEstadorEquipo()" id="btn-tipoEstadoEquipo" class="btn btn-info btn-raised btn-sm"><i class="zmdi zmdi-floppy"></i> Save</button>
										    </p>
									    </form>
									</div>
								</div>
					  	...
					  	<div class="tab-pane fade" id="list">
							<div class="table-responsive">
								<table id="id_tabla_estado_equipo" style="width: 100%" class="display table table-hover text-center">
									<thead>
										<tr>
											<th class="text-center">Estados de Equipos</th>
											<th class="text-center">Editar</th>
											<th class="text-center">Eliminar</th>
										</tr>
									</thead>
									<tbody>
										<?php foreach($querySelectEstadoEquipo as $fila){ ?>
										<tr>
											<td><?php echo utf8_encode($fila['tipoestado']) ?></td>
											<td><a onclick="EditarEstadoEquipo('<?php echo $fila['idtipoestado'] ?>')" data-toggle="modal" data-target="#ModalEstadoEquipo"   class="btn btn-success btn-raised btn-xs"><i class="zmdi zmdi-refresh"></i></a></td>
											<td><button type="button" onclick="EliminarEstadoEquipo(<?php echo $fila['idtipoestado'] ?>)"  class="btn btn-danger btn-raised btn-xs"><i class="zmdi zmdi-delete"></i></a></td>
										</tr>
										<?php } ?>
									</tbody>
								</table>
...
<!-- Modal -->
<div class="modal fade" id="ModalEstadoEquipo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h2 class="modal-title" id="exampleModalLabel">Editar Estado de Equipos</h2>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
	  <input type="hidden"  class="form-control"  id="txtIdEstadoEquipoEditar">
	  	<div class="form-group label-floating">
			<input class="form-control" id="txtEstadoEquipoEditar" autocomplete="off">
		</div>
      </div>
      <div class="modal-footer">
        <button type="button"  class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
        <button type="button" onclick="ActualizarEstadoEquipo()" class="btn btn-primary">Guardar Cambios</button>
      </div>
    </div>
  </div>
</div>
	...
	<script src="../Controlador/ControladorTipoEstadoEquipo.js"></script>
	<script>
		$.material.init();
	</script>
</body>
