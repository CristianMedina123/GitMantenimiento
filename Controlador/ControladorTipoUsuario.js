...
function EliminarTipoUsuario(id) {
  ...
  $.ajax({
    type: "POST",
    url: "../Modelo/EliminarTipoUsuario.php",
    data: cadena,
    success: function (res) {
      if (res == 1) {
...
function EditarTipoUsuario(id) {
  ...
  $.ajax({
    type: "POST",
      url: "../Modelo/MostrarTipoUsuario.php",
      dataType: "json",
      data: cadena,
      success: function (res){
...
function ActualizarTipoUsuario() {
  var id = $("#txtIdTipoUsuario").val();
  var tipo = $("#txtTipoUsuarioEditar").val();

  if(tipo.length < 3 || tipo.length === 0){
    alertify.alert('Ocurrió un error','No puede dejar campos vacíos', function(){ alertify.error('¡Error!'); }).set('closable', false);
  }else{
    cadena = "id=" + id + "&tipo=" + tipo;
    $.ajax({
      type: "POST",
        url: "../Modelo/ActualizarTipoUsuario.php",
        data: cadena,
        success: function (res)
        {
            if (res == 1) {
                $("#ModalTipoUsuarioEditar").modal("hide");
...
$(document).ready(function () {
  $('#id_tabla_tipousuario').DataTable({
      language: {
          aria: {
